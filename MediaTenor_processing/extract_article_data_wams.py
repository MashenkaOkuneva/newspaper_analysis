# -*- coding: utf-8 -*-
"""
Created on Sat Jun 29 10:46:27 2024
...
def extract_article_data_wams(wams_txt_path):
    """
    Read and extract relevant information from WamS articles downloaded from Factiva.
    
    Parameters:
    wams_txt_path (str): Path to WamS TXT files.
    ...
    for f in os.listdir(wams_txt_path):
        # Ignore .gitkeep files
        if f == '.gitkeep':
            continue
        # Open each TXT file with UTF-8 encoding
        with codecs.open(os.path.join(wams_txt_path, f), "r", encoding='utf-8') as input_data:
            files.append(f)
            text = []
            author = 'null author'
            ...
            for line in input_data:
                ...
                if line and line not in ["Welt der Wirtschaft", "Titel", "Invest", "Start", "Editorial", "Leben",
                                        "Politik", "Wirtschaft", "32", "POLITIK", "WIRTSCHAFT", "TITEL", "FORUM",
                                        "TITELTHEMA", "SONDERTHEMEN", "FINANZEN"]:
                    titles.append(line)
                    break
            ...
            for line in input_data:
                if any(keyword in line for keyword in ['Welt am Sonntag', 'WELT online']):
                    journal.append('WamS')
                    break
