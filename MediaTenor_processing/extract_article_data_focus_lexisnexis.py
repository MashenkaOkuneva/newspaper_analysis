# -*- coding: utf-8 -*-
"""
Created on Wed Jun 26 18:38:42 2024
...
def extract_article_data_focus_lexisnexis(focus_lexisnexis_txt_path):
    """
    Read and extract relevant information from Focus articles downloaded from LexisNexis.
    
    Parameters:
    focus_lexisnexis_txt_path (str): Path to Focus TXT files.
    ...
    for f in os.listdir(focus_lexisnexis_txt_path):
        # Ignore .gitkeep files
        if f == '.gitkeep':
            continue
        # Open each TXT file with UTF-8 encoding
        with codecs.open(os.path.join(focus_lexisnexis_txt_path, f), "r", encoding='utf-8') as input_data:
            files.append(f)
            text = []
            author = 'null author'
