...
run_midas_pca <- function(p) {
  ...
  write.table(nowcasts_df, sprintf("nowcasts_midas_pca_%dlags_hard.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_1step_df, sprintf("forecasts_1step_midas_pca_%dlags_hard.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_2step_df, sprintf("forecasts_2step_midas_pca_%dlags_hard.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
...
calculate_forecast_errors <- function(results, p) {
  ...
  file_name <- sprintf("forecast_errors_midas_pca_%dlags_hard.csv", p)
...
forecasts_midas_plot <- function(results, p) {
  ...
  generate_plot(nowcasts, "Nowcasts vs. GDP Growth", sprintf("nowcasts_plot_midas_pca_%dlags_hard.pdf", p))
  generate_plot(forecasts_1step, "One-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_1step_plot_midas_pca_%dlags_hard.pdf", p))
  generate_plot(forecasts_2step, "Two-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_2step_plot_midas_pca_%dlags_hard.pdf", p))
...
generate_latex_table <- function(comparison_results, comparison_type, test_type, file_name) {
  ...
  for (p in 0:p_max) {
    ...
    row <- c(paste("hard, sign1, PCA ($p = ", p, "$)", sep = ""), formatted_rmse)
...
for (comparison_type in comparison_types) {
  for (test_type in test_types) {
    file_name <- paste0("pca_hard_", tolower(comparison_type), "_", test_type, ".tex")
