...
prepare_vintage <- function(vintage, date_h, sample_start = c("1992-01-01"), Ntopics = 4,
  ...
  # Left join the df_gdp with all_dates to get a monthly dataframe
  df_gdp <- dates_tmp %>%
  ...
  # export to csv
  write.csv(df_data, file = paste0("./vintages_MIDAS/", "vint_", year(vintage), "_", month(vintage), "_", day(vintage), ".csv"),
            row.names = F,
            quote = F,
            na = "NaN")
}
...
if(!dir.exists("./vintages_MIDAS")) {
  dir.create("./vintages_MIDAS")
}
