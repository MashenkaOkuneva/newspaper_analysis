...
#_This script prepares a dataset for macroeconomic forecasting.  
# It performs several key tasks: 
# 1. Downloading and transforming economic variables.
# 2. Preparing for the forecast by appending rows and creating the Xi_md and Xi_qd indicators.
# 3. Calculating weights for monthly and quarterly flow variables.
# 4. Finally, the script generates indicators for nowcasting 
...
prepare_vintage <- function(vintage, date_h, sample_start = c("1992-01-01"),
  ...
  # if no transformations for financial variables
  objects_to_remove <- c(
  ...
  # convert to "daily" frequency
  dates_tmp <- data.frame(date = seq(min(economic_data$date), 
                                     max(economic_data$date), 
                                     by = "days")
  )
  ...
  # EXPORT TO CSV ----
  #_____________________________________________________#
  #_indices for now- and forecasts
  #_merge datasets
  ...
  ind_d <- grepl("financial.", names(df_data))
  Nd <- sum(ind_d)
  ...
  # export to csv
  write.csv(df_data, file = paste0("./vintages_hard/", "vint_", year(vintage), "_", month(vintage), "_", day(vintage), ".csv"),
            row.names = F,
            quote = F,
            na = "NaN")
}
...
# Generate vintages
vintages <- seq(as.Date("2010-03-30"), as.Date("2018-06-30"), by = "quarter")
...
if(!dir.exists("./vintages_hard")) {
  dir.create("./vintages_hard")
}
