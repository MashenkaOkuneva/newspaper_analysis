...
prepare_vintage <- function(vintage, date_h, sample_start = c("1992-01-01"),
  ...
  # export to csv
  write.csv(df_data, file = paste0("./vintages_hard_MIDAS/", "vint_", year(vintage), "_", month(vintage), "_", day(vintage), ".csv"),
            row.names = F,
            quote = F,
            na = "NaN")
}
...
if(!dir.exists("./vintages_hard_MIDAS")) {
  dir.create("./vintages_hard_MIDAS")
}
