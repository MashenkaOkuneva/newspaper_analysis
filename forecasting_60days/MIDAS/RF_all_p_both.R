...
# hard data and text data, with Random Forests applied to handle dimensionality reduction 
# and capture nonlinear relationships between the predictors and the target variable. 
# Forecasts are generated across different forecasting horizons (nowcasts, 
# 1-step-ahead, and 2-step-ahead forecasts) and for varying numbers of lags (p). The same 
# number of lags is applied uniformly across all predictors, though the starting point may 
# differ depending on data availability for the hard and text-based series.
#
...
# is performed to compare the MIDAS-Random Forest forecasts against AR(1) and Reuters poll forecasts.
...
run_midas_rf <- function(p) {
  ...
  write.table(nowcasts_df, sprintf("nowcasts_midas_rf_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_1step_df, sprintf("forecasts_1step_midas_rf_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_2step_df, sprintf("forecasts_2step_midas_rf_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
...
calculate_forecast_errors <- function(results, p) {
  ...
  file_name <- sprintf("forecast_errors_midas_rf_%dlags_both_10stable.csv", p)
...
forecasts_midas_plot <- function(results, p) {
  ...
  generate_plot(nowcasts, "Nowcasts vs. GDP Growth", sprintf("nowcasts_plot_midas_rf_%dlags_both_10stable.pdf", p))
  generate_plot(forecasts_1step, "One-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_1step_plot_midas_rf_%dlags_both_10stable.pdf", p))
  generate_plot(forecasts_2step, "Two-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_2step_plot_midas_rf_%dlags_both_10stable.pdf", p))
...
generate_latex_table <- function(comparison_results, comparison_type, test_type, file_name) {
  ...
  for (p in 0:p_max) {
    ...
    row <- c(paste("both, sign1, RF ($p = ", p, "$)", sep = ""), formatted_rmse)
...
for (p in 0:p_max) {
  # Run the MIDAS regression with RF
  results <- run_midas_rf(p = p)
  
  # Calculate forecast errors
  fe_midas_rf = calculate_forecast_errors(results = results, p = p)
  
  # Generate plots
  forecasts_midas_plot(results = results, p = p)
  
  # Run the Diebold-Mariano test
  ar1_two_sided <- compare_forecast_errors(fe_midas_rf, comparison_type = "AR1", test_type = "two-sided")
  ar1_one_sided <- compare_forecast_errors(fe_midas_rf, comparison_type = "AR1", test_type = "one-sided")
  professional_two_sided <- compare_forecast_errors(fe_midas_rf, comparison_type = "professional", test_type = "two-sided")
  professional_one_sided <- compare_forecast_errors(fe_midas_rf, comparison_type = "professional", test_type = "one-sided")
...
for (comparison_type in comparison_types) {
  for (test_type in test_types) {
    file_name <- paste0("rf_both_10stable_", tolower(comparison_type), "_", test_type, ".tex")
