...
# (nowcasts, 1-step-ahead, and 2-step-ahead forecasts) and for varying numbers of lags (p).
...
run_midas_pca <- function(p) {
  ...
  for (file in files) {
    ...
    nowcast_result <- perform_forecast(data, nlag = 1, p = p, mon = 2, forecast_indicator = 'ind_nowcast')
    one_step_result <- perform_forecast(data, nlag = 2, p = p, mon = 2, forecast_indicator = 'ind_forecast1Q')
    two_step_result <- perform_forecast(data, nlag = 3, p = p, mon = 2, forecast_indicator = 'ind_forecast2Q')
...
generate_latex_table <- function(comparison_results, comparison_type, test_type, file_name) {
  ...
  for (p in 0:p_max) {
    ...
    row <- c(paste("both, sign1, PCA, 2 factors ($p = ", p, "$)", sep = ""), formatted_rmse)
