...
# hard data and text data, with Ridge applied for dimensionality reduction. Forecasts are 
...
perform_forecast <- function(data, nlag, p, mon, forecast_indicator, is_backcast = FALSE) {
  ...
  # Fit the Ridge model on the estimation data
  out = glmnet(estimation_data, y, alpha = 0, lambda=grid)
  
  # Generate forecast using the Ridge model
...
run_midas_ridge <- function(p) {
  ...
  write.table(nowcasts_df, sprintf("nowcasts_midas_ridge_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_1step_df, sprintf("forecasts_1step_midas_ridge_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_2step_df, sprintf("forecasts_2step_midas_ridge_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
...
calculate_forecast_errors <- function(results, p) {
  ...
  file_name <- sprintf("forecast_errors_midas_ridge_%dlags_both_10stable.csv", p)
...
forecasts_midas_plot <- function(results, p) {
  ...
  generate_plot(nowcasts, "Nowcasts vs. GDP Growth", sprintf("nowcasts_plot_midas_ridge_%dlags_both_10stable.pdf", p))
  generate_plot(forecasts_1step, "One-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_1step_plot_midas_ridge_%dlags_both_10stable.pdf", p))
  generate_plot(forecasts_2step, "Two-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_2step_plot_midas_ridge_%dlags_both_10stable.pdf", p))
...
generate_latex_table <- function(comparison_results, comparison_type, test_type, file_name) {
  ...
  for (p in 0:p_max) {
    ...
    row <- c(paste("both, sign1, Ridge ($p = ", p, "$)", sep = ""), formatted_rmse)
...
for (comparison_type in comparison_types) {
  for (test_type in test_types) {
    file_name <- paste0("ridge_both_10stable_", tolower(comparison_type), "_", test_type, ".tex")
