...
# are generated across different forecasting horizons (backcasts, nowcasts, 1-step-ahead, and 
...
run_midas_rf <- function(p) {
  ...
  write.table(backcasts_df, sprintf("backcasts_midas_rf_%dlags_hard.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
...
forecasts_midas_plot <- function(results, p) {
  ...
  generate_plot(backcasts, "Backcasts vs. GDP Growth", sprintf("backcasts_plot_midas_rf_%dlags_hard.pdf", p))
