...
# where LASSO is used for dimensionality reduction. Forecasts are generated across different 
...
run_midas_lasso <- function(p) {
  ...
  write.table(backcasts_df, sprintf("backcasts_midas_lasso_10stable_%dlags.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(nowcasts_df, sprintf("nowcasts_midas_lasso_10stable_%dlags.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_1step_df, sprintf("forecasts_1step_midas_lasso_10stable_%dlags.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_2step_df, sprintf("forecasts_2step_midas_lasso_10stable_%dlags.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
...
calculate_forecast_errors <- function(results, p) {
  ...
  file_name <- sprintf("forecast_errors_midas_lasso_10stable_%dlags.csv", p)
...
forecasts_midas_plot <- function(results, p) {
  ...
  generate_plot(backcasts, "Backcasts vs. GDP Growth", sprintf("backcasts_plot_midas_lasso_10stable_%dlags.pdf", p))
  generate_plot(nowcasts, "Nowcasts vs. GDP Growth", sprintf("nowcasts_plot_midas_lasso_10stable_%dlag.pdf", p))
  generate_plot(forecasts_1step, "One-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_1step_plot_midas_lasso_10stable_%dlags.pdf", p))
  generate_plot(forecasts_2step, "Two-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_2step_plot_midas_lasso_10stable_%dlags.pdf", p))
...
generate_latex_table <- function(comparison_results, comparison_type, test_type, file_name) {
  ...
  for (p in 0:p_max) {
    ...
    row <- c(paste("10 Topics, sign1, LASSO ($p = ", p, "$)", sep = ""), formatted_rmse)
...
for (comparison_type in comparison_types) {
  for (test_type in test_types) {
    file_name <- paste0("lasso_10stable_", tolower(comparison_type), "_", test_type, ".tex")
