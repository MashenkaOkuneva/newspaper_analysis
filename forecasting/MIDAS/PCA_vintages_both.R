# This script performs forecasting using a MIDAS model that combines both text data and hard economic data. 
# Principal Component Analysis (PCA) is used for dimensionality reduction, applying the EM algorithm from Stock 
# and Watson (2002) to extract common factors from both data sources.
...
for (file in files) {
  ...
  backcast_result <- perform_forecast(data, nlag = 0, p = 0, mon = 1, forecast_indicator = 'ind_backcast', is_backcast = TRUE)
...
write.table(backcasts_df, "backcasts_midas_pca_both.csv", row.names = FALSE, col.names=FALSE, sep=",", quote=FALSE)
write.table(nowcasts_df, "nowcasts_midas_pca_both.csv", row.names = FALSE, col.names=FALSE, sep=",", quote=FALSE)
write.table(forecasts_1step_df, "forecasts_1step_midas_pca_both.csv", row.names = FALSE, col.names=FALSE, sep=",", quote=FALSE)
write.table(forecasts_2step_df, "forecasts_2step_midas_pca_both.csv", row.names = FALSE, col.names=FALSE, sep=",", quote=FALSE)