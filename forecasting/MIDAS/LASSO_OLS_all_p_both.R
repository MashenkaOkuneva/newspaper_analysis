...
# (backcasts, nowcasts, 1-step-ahead, and 2-step-ahead forecasts) and for varying numbers 
...
run_midas_lasso_ols <- function(p) {
  ...
  write.table(backcasts_df, sprintf("backcasts_midas_lasso_ols_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
...
forecasts_midas_plot <- function(results, p) {
  ...
  generate_plot(backcasts, "Backcasts vs. GDP Growth", sprintf("backcasts_plot_midas_lasso_ols_%dlags_both_10stable.pdf", p))
