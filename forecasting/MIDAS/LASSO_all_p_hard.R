...
run_midas_lasso <- function(p) {
  ...
  write.table(backcasts_df, sprintf("backcasts_midas_lasso_%dlags_hard.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
...
forecasts_midas_plot <- function(results, p) {
  ...
  generate_plot(backcasts, "Backcasts vs. GDP Growth", sprintf("backcasts_plot_midas_lasso_%dlags_hard.pdf", p))
