# This script implements a forecasting experiment using a MIDAS model that combines both 
# hard data and text data, with Principal Component Analysis (PCA) applied for dimensionality reduction. 
# Common factors are extracted jointly from the text and financial/economic series using the EM 
# algorithm of Stock and Watson (2002). Forecasts are generated across different forecasting horizons 
# (backcasts, nowcasts, 1-step-ahead, and 2-step-ahead forecasts) and for varying numbers of lags (p).
# The same number of lags is applied uniformly across all factors.
#
...
# is performed to compare the MIDAS-PCA forecasts against AR(1) and Reuters poll forecasts.
...
perform_forecast <- function(data, nlag, p, mon, forecast_indicator, is_backcast = FALSE) {
  ...
  # Function Parameters:
  # data: The dataframe coresponding to a particular vintage, containing GDP growth, topics data and hard economic data.
  ...
  selected_columns <- names(data)[startsWith(names(data), "T") | startsWith(names(data), "financial") | startsWith(names(data), "economic")]
  ...
  all_series <- data_rm_na[, selected_columns]
  
  # PCA based on EM algorithm following Stock and Watson (2002).
  #pca <- f_emalg(scale(all_series), Nr_max = 10, Niter = 50, ic = "PC_p1",  print_iter = TRUE)
  # Manually set number of factors to 2!
  pca <- f_emalg(scale(all_series), Nr_max = 2, Niter = 50, ic = "none",  print_iter = TRUE)
  ...
  # Removing the rest of the series
...
run_midas_pca <- function(p) {
  
  # Get the list of all .csv files in the 'vintages_MIDAS' directory
  files <- list.files(path="../hard_data/vintages_both_MIDAS/", pattern="*.csv")
  ...
  for (file in files) {
    ...
    # Read in the data
    data <- read.csv(paste0("../hard_data/vintages_both_MIDAS/", file), header = TRUE, stringsAsFactors = FALSE)
    
    backcast_result <- perform_forecast(data, nlag = 0, p = p, mon = 1, forecast_indicator = 'ind_backcast', is_backcast = TRUE)
  ...
  write.table(backcasts_df, sprintf("backcasts_midas_pca_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(nowcasts_df, sprintf("nowcasts_midas_pca_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_1step_df, sprintf("forecasts_1step_midas_pca_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
  write.table(forecasts_2step_df, sprintf("forecasts_2step_midas_pca_%dlags_both_10stable.csv", p), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
...
calculate_forecast_errors <- function(results, p) {
  ...
  file_name <- sprintf("forecast_errors_midas_pca_%dlags_both_10stable.csv", p)
...
forecasts_midas_plot <- function(results, p) {
  ...
  generate_plot(backcasts, "Backcasts vs. GDP Growth", sprintf("backcasts_plot_midas_pca_%dlags_both_10stable.pdf", p))
  generate_plot(nowcasts, "Nowcasts vs. GDP Growth", sprintf("nowcasts_plot_midas_pca_%dlags_both_10stable.pdf", p))
  generate_plot(forecasts_1step, "One-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_1step_plot_midas_pca_%dlags_both_10stable.pdf", p))
  generate_plot(forecasts_2step, "Two-step-ahead Forecasts vs. GDP Growth", sprintf("forecasts_2step_plot_midas_pca_%dlags_both_10stable.pdf", p))
...
generate_latex_table <- function(comparison_results, comparison_type, test_type, file_name) {
  ...
  for (p in 0:p_max) {
    ...
    row <- c(paste("both, sign1, PCA ($p = ", p, "$)", sep = ""), formatted_rmse)
...
for (p in 0:p_max) {
  # Run the MIDAS regression with PCA
  results <- run_midas_pca(p = p)
  
  # Calculate forecast errors
  fe_midas_pca = calculate_forecast_errors(results = results, p = p)
  
  # Generate plots
  forecasts_midas_plot(results = results, p = p)
  
  # Run the Diebold-Mariano test
  ar1_two_sided <- compare_forecast_errors(fe_midas_pca, comparison_type = "AR1", test_type = "two-sided")
  ar1_one_sided <- compare_forecast_errors(fe_midas_pca, comparison_type = "AR1", test_type = "one-sided")
  professional_two_sided <- compare_forecast_errors(fe_midas_pca, comparison_type = "professional", test_type = "two-sided")
  professional_one_sided <- compare_forecast_errors(fe_midas_pca, comparison_type = "professional", test_type = "one-sided")
...
for (comparison_type in comparison_types) {
  for (test_type in test_types) {
    file_name <- paste0("pca_both_10stable_", tolower(comparison_type), "_", test_type, ".tex")
